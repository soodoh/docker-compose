...
Activation produced Renovate PR [`#214`](https://github.com/soodoh/docker-compose/pull/214), which changes only `services/servarr.yml`, carries `dependencies` and `compose-canary`, passed the required deterministic artifact check, and has bot-owned squash automerge queued. The `renovate/stability-days` check remains pending for the configured seven-day release age. This is the expected fail-closed state: no image has been pulled and no container has been recreated yet. Once the age check passes, the merge-triggered deployment must independently prove the one-file, one-image, one-recreation, stateless canary policy before the apply job can run.
