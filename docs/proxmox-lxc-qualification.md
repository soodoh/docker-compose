...
Before create, read-only preflight requires the protected VMID and all matching volumes to be absent and the exact template to exist once. Every later phase requires the canonical fixed marker (the provider/API's single trailing newline is accepted and removed only for comparison), stopped status, disabled console, no network, no mount points or additional disks, no passthrough, mappings, hooks, tags, startup behavior, or non-default features, the minimal local-LVM root disk, exact protection state, and agreement between protected inputs, API, and isolated state. The helper rejects any extra capability exposed in the pinned provider plan/state or Proxmox configuration. It never broadens ACLs and never discovers or chooses an identifier.
