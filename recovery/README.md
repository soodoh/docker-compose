...
For a non-S3 rehearsal only, `playbooks/disaster-recovery.yml` accepts an exact local archive under `/mnt/storage/backups/` when `recovery_local_rehearsal_confirmation` equals `use-reviewed-local-backup-without-s3-download`. The local archive is hashed, decrypted, and fully extracted through the same safety path, but this mode cannot satisfy remote RPO or qualify S3 transport.
...
CT 101 is adopted in its separate protected state. It is neither rebuilt nor decommissioned by recovery mode. Only after direct Tailscale paths and every other OpenTofu/Ansible/Compose check are no-ops, provide the protected `PROXMOX_CT_DECOMMISSION_CONFIRMATION` environment secret. In separate reviewed changes, move the durable contract stage from `protected` to `unprotected` and run `--phase steady --ct-operation unprotect`; require a converged `--phase steady --ct-operation none` proof before moving from `unprotected` to `retired` and running `--phase steady --ct-operation delete`. Finish with another `none` proof. The exact operation and stage are bound into the saved-plan manifest, the policy permits only the matching CT 101 action, and the orchestrator applies that action last. The retired stage keeps resource count and import disabled so steady reconciliation cannot recreate the container.
